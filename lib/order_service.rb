...
module  OrderService
  ...
  def self.find_or_create_patient(npid, params)
    patient_obj = Patient.find_by_id(Specimen.where(tracking_number: params[:tracking_number])
                         .joins(:tests).pluck('tests.patient_id')) || Patient.find_by_patient_number(params[:national_patient_id])
    if patient_obj.blank?
      patient_obj = Patient.create!(patient(npid, params))
    else
      update_dated = patient_obj.update(patient(npid, params))
      patient_obj = Patient.find_by_patient_number(params[:national_patient_id]) if update_dated
    end
    patient_obj
  end

  def self.patient(npid, params)
    {
      patient_number: npid,
  ...
  def self.create_specimen(tracking_number, common_data, params)
    specimen = Specimen.find_by_tracking_number(tracking_number)
    return specimen unless specimen.blank?
    
    Specimen.find_or_create_by!(
  ...
  def self.create_visit(patient_id, ward_id)
    Visit.find_or_create_by(patient_id: patient_id, ward_id: ward_id)
  end
  ...
  def self.create_tests(tests, sp_obj, patient_obj, common_data)
    ...
    tests.each do |tst|
      ...
      if panel_test?(tst) == false
        test_status[tst] = details
        create_test_for_type(sp_obj, tst, patient_obj, common_data)
      else
        create_tests_for_panel_types(sp_obj, tst, patient_obj, common_data)
      end
    end
  end

  def self.create_test_for_type(sp_obj, tst, patient_obj, common_data)
  ...
  def self.create_tests_for_panel_types(sp_obj, tst, patient_obj, common_data)
    ...
    test_types.each do |tt|
      create_test_for_panel_type(sp_obj, tt, patient_obj, common_data)
    end
  end

  def self.create_test_for_panel_type(sp_obj, tt, patient_obj, common_data)
    test_status_id = TestStatus.get_test_status_id('drawn')
  ...
  def self.check_order(tracking_number)
    order = Specimen.where(tracking_number: tracking_number).first
    if order
      true
    else
      false
  ...
  def self.query_order_by_tracking_number_v2(tracking_number,test_name)
    ...
    res = Specimen.find_by_sql("SELECT specimen_types.name AS sample_type, specimen_statuses.name AS specimen_status,
  ...
  def self.get_site_code_number(site_code_alpha)
      ...
      res = Specimen.find_by_sql("SELECT sending_facility FROM specimen WHERE tracking_number='#{site_code_alpha}'")
      if !res.blank?
  ...
  def self.get_order_by_tracking_number_sql(track_number)
    details = Specimen.where(tracking_number: track_number).first
    details.blank? ? false : details
  end

  def self.query_results_by_npid(npid)

    ord = Specimen.find_by_sql("SELECT specimen.id AS trc, specimen.tracking_number AS track,specimen_types.name AS spec_name FROM specimen
      ...
      ord.each do |ord_lo|
        ...
        if r.length > 0
          ...
          r.each do |te|
            ...
            res = Specimen.find_by_sql( "SELECT measures.name AS measure_name, test_results.result AS result,
                                                      tests.id AS tstt_id
  ...
  def self.retrieve_undispatched_samples(facilities)
    ...
    facilities.each do |facility|
      ...
      if !res.blank?
        res_ = Specimen.find_by_sql("SELECT specimen.tracking_number AS tracking_number, specimen_types.name AS sample_type, specimen_statuses.name AS specimen_status,
                                                      wards.name AS order_location, specimen.date_created AS date_created, specimen.priority AS priority,
                                                      specimen.drawn_by_id AS drawer_id, specimen.drawn_by_name AS drawer_name,
                                                      specimen.drawn_by_phone_number AS drawe_number, specimen.target_lab AS target_lab,
                                                      specimen.sending_facility AS health_facility, specimen.requested_by AS requested_by,
  ...
  def self.retrieve_samples(date,region)
    orders = Specimen.find_by_sql("SELECT specimen_types.name AS sample_type, specimen_statuses.name AS specimen_status,
  ...
  def self.request_order(params,tracking_number)
    ...
    ActiveRecord::Base.transaction do
      ...
      sp = Specimen.find_by(:tracking_number => tracking_number)
  ...
  def self.confirm_order_request(ord)
    ...
    obj = Specimen.find_by(:tracking_number => ord['tracking_number'])
  ...
  def self.query_requested_order_by_npid(npid)
    ...
    r = Specimen.find_by_sql("SELECT distinct(specimen.id) FROM specimen INNER JOIN tests ON specimen.id = tests.specimen_id INNER JOIN patients ON patients.id = tests.patient_id WHERE patients.patient_number = '#{npid}'")
    ...
    if r.length > 0
      ...
      r.each do |data|
        ...
        if !checker.include?(tra_num)
          da = Specimen.find_by_sql("SELECT * FROM specimen WHERE id='#{tra_num}'")
  ...
  def self.query_order_by_npid(npid)
    ...
    (tests || []).each do |tst|
      next if TestResult.find_by_test_id(tst.id).present?
      test_type = TestType.find(tst.test_type_id)
      test_status = TestStatus.find(tst.test_status_id)
      tracking_number = Specimen.find(tst.specimen_id).tracking_number
  ...
  def self.panel_test?(tst)
    PanelType.find_by_name(tst) ? true : false
  end

  def self.check_if_order_updated?(tracking_number,status_id)
    obj = Specimen.find_by(:tracking_number => tracking_number ,:specimen_status_id => status_id)
    if !obj.blank?
      true
    else
      false
  ...
  def self.update_order(ord)
    ...
    if !ord['specimen_type'].blank?
      ...
      return [false,"wrong parameter for specimen type"] unless (!sp_type.blank?)
